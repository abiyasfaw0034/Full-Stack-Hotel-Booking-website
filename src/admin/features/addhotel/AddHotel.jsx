...
// you have to add the isloading to disable inputs
...
function AddHotel() {
  ...
  const handleCheckboxChange = (event) => {
    ...
    setSelectedFacilities((prev) =>
      checked ? [...prev, name] : prev.filter((facility) => facility !== name)
    );
  };
...
// Ensure other components like ImageUpload, AddRoom, HotelFacilities, and MapPage are defined as per their usage

function ImageUpload({ onFileChange, imageFiles, setImageFiles }) {
...
function LocationModal({ isOpen, onClose, onLocationSelect }) {
  ...
  useEffect(() => {
    if (isOpen && mapRef.current) {
      ...
      }, 100); // Delay to ensure the map has fully loaded
    }
  }, [isOpen]);
